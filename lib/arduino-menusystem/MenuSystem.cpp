...
bool Menu::next(bool loop) {
    ...
    if (!_num_components) {
        ...
        // FIX: Moved line to stop next() from setting _is_current
        // to false if there were only one MenuComponent in a Menu.
        _menu_components[_previous_component_num]->set_current(false);
...
Menu* Menu::select() {
    ...
    // TODO: block directly in Menu::Activate instead, and make
    // Menu::block_access() a setter instead (less confusing solution).

    // FIX: _block_access can be set in this Menus _select_fn that is called
    // on the previous line. By returning a nullptr here _p_curr_menu in the
    // MenuSystem is not updated, effectivly blocking access to this Menu.
    if (_block_access) {
      _block_access = false;
      return nullptr;
    }
...
// FIX: When block_access is true, the Menu should not be selectable.
void Menu::block_access() {
    _block_access = true;
}
...
bool MenuSystem::back() {
    // FIX: Unfocus current MenuComponent when calling back().
    if (_p_curr_menu->_p_current_component->has_focus())
