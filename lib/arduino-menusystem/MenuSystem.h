...
public:
    ...
    // FIX: Sets _block_access to true. This
    void block_access();
...
private:
    ...
    // FIX: Added bool _block_access, which makes it possible to block access
    // to Menus from their select_fn (that is called after select() is called).
    bool _block_access;
};
